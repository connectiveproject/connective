...
@pytest.fixture
def mock_event_now_time():
    return mock.patch("events.models.timezone.now")
...
class TestEventOrderSignals:
    ...
    def test_event_recurring_numbered(self, all_entities, mock_event_now_time):
        mock_event_now_time.return_value = datetime(2020, 1, 1, tzinfo=timezone.utc)
        order_pending_approval = EventOrder.objects.create(
            status=EventOrder.Status.PENDING_APPROVAL,
            recurrence=EventOrder.Recurrence.ONE_TIME,
            school_group=all_entities["activity_group"],
        ...
        EventOrder.objects.create(  # one time order
            status=EventOrder.Status.APPROVED,
            recurrence=EventOrder.Recurrence.ONE_TIME,
            school_group=all_entities["activity_group"],
            start_time=today,
        ...
        order_weekly = EventOrder.objects.create(
            status=EventOrder.Status.APPROVED,
            recurrence=EventOrder.Recurrence.WEEKLY_NUMBERED,
            school_group=all_entities["activity_group"],
            start_time=today,
            end_time=tomorrow,
        )
        events_count_53 = Event.objects.filter(
            event_order__in=[order_pending_approval, order_weekly]
        ).count()

        mock_event_now_time.return_value = datetime(
